...
def _generate_with_openai(client, client_name, competitors_data):
    ...
    Week: {week_str}
    Data: {context}
    Return JSON: {{"week":"{week_str}","summary":"2-3 sentence summary","competitors":[{{"name":"","signal":"high|medium|low|opportunity","headline":"","detail":"","watch":null,"ad_count":0,"tags":[]}}],"recommendations":[{{"priority":1,"title":"","detail":""}}]}}"""
    ...
    comps = []
        ...
        comps.append({"name": n, "signal": sig, "headline": f"{n}: {sig.upper()} activity",
                              "detail": f"Observed {count} ads in the past 7 days.",
                                                    "watch": f"Monitor {n} for shifts" if count > 0 else None,
                                                                          "ad_count": count, "tags": ["Active" if count > 5 else "Low"]})
                                                                              return {
                                                                                      "week": week_str,
